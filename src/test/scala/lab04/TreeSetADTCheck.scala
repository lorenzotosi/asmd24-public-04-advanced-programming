...
import scala.lab04.SetADTs.{SetADT, TreeBasedSetADT}

abstract class TreeSetADTCheck(name: String) extends Properties(name):
  val setADT: SetADT
  import setADT.*
  ...
  property("absorption laws") =
    forAll: (s1: Set[Int], s2: Set[Int]) =>
      (s1 || (s1 && s2)) === s1 && (s1 && (s1 || s2)) === s1


object MyTreeSetADTCheck extends TreeSetADTCheck("TreeBased Set"):
  val setADT: SetADT = TreeBasedSetADT

  @main def visuallingCheckArbitrarySets1 =
